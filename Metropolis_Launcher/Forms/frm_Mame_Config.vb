...
Public Class frm_Mame_Config
	...
	Private Sub btn_Scan_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_Scan.Click
		If txb_MameExe.Text = "" Then
			DevExpress.XtraEditors.XtraMessageBox.Show("Please provide the full path to your mame.exe or mame64.exe.", "M.A.M.E. Executable missing", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			txb_MameExe.Focus()
			Return
		End If

		If Not Alphaleonis.Win32.Filesystem.File.Exists(txb_MameExe.Text) Then
			DevExpress.XtraEditors.XtraMessageBox.Show("M.A.M.E. Executable is missing!", "M.A.M.E. Executable missing", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			txb_MameExe.Focus()
			Return
		End If

		If Not Alphaleonis.Win32.Filesystem.File.Exists(txb_MameExe.Text) Then
			DevExpress.XtraEditors.XtraMessageBox.Show("M.A.M.E. Executable is missing!", "M.A.M.E. Executable not found", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			txb_MameExe.Focus()
			Return
		End If
		...
		If Not Alphaleonis.Win32.Filesystem.File.Exists(Alphaleonis.Win32.Filesystem.Path.GetDirectoryName(txb_MameExe.Text) & "\mame.ini") Then
			DevExpress.XtraEditors.XtraMessageBox.Show("The mame.ini file is missing, please create one and configure it. The command is '" & txb_MameExe.Text & " -cc'", "mame.ini not found", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
			txb_MameExe.Focus()
			Return
		End If

		If txb_history_dat.Text = "" Then
			If Not DevExpress.XtraEditors.XtraMessageBox.Show("Although optional, it is recommended that you provide a history.dat file that fits your M.A.M.E. installation as it provides interesting information for games. Download history.dat from http://www.arcade-history.com/. Do you still want to continue the scan process?", "history.dat is missing", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
				txb_history_dat.Focus()
				Return
			End If
		ElseIf Not Alphaleonis.Win32.Filesystem.File.Exists(txb_history_dat.Text) Then
			If Not DevExpress.XtraEditors.XtraMessageBox.Show("The history.dat file you provided cannot be found! Do you still want to continue the scan process?", "history.dat not found", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
				txb_history_dat.Focus()
				Return
			End If
		End If
		...
		If txb_mameinfo_dat.Text = "" Then
			If Not DevExpress.XtraEditors.XtraMessageBox.Show("Although optional, it is recommended that you provide a mameinfo.dat file that fits your M.A.M.E. installation as it provides interesting information for games. Download mameinfo.dat from http://mameinfo.mameworld.info/. Do you still want to continue the scan process?", "mameinfo.dat is missing", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
				txb_mameinfo_dat.Focus()
				Return
			End If
		ElseIf Not Alphaleonis.Win32.Filesystem.File.Exists(txb_mameinfo_dat.Text) Then
			If Not DevExpress.XtraEditors.XtraMessageBox.Show("The mameinfo.dat file you provided cannot be found! Do you still want to continue the scan process?", "mameinfo.dat not found", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
				txb_mameinfo_dat.Focus()
				Return
			End If
		End If

		If Not DevExpress.XtraEditors.XtraMessageBox.Show("The scan process will gather game informations from your provided M.A.M.E. installation and will scan for games in your M.A.M.E. rom directory. This will take a while, do you want to continue?", "Scan", MessageBoxButtons.YesNo, MessageBoxIcon.Information) = Windows.Forms.DialogResult.Yes Then
			Return
		End If
		...
		'Dim res As DialogResult = DevExpress.XtraEditors.XtraMessageBox.Show("Fetch Rom infos from Mame, history.dat and mameinfo?", "Question", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question)
		...
		'res = DevExpress.XtraEditors.XtraMessageBox.Show("Verify roms from M.A.M.E. and add games?", "Question", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question)
		...
		Dim prg As New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(Nothing), 400, 60, ProgressBarStyle.Marquee, False, "Scan step 6 of 7: Verifying roms (please be patient, this can take a while!)", 0, 100, True)
		prg.Start()
		...
		Using tran = cls_Globals.Conn.BeginTransaction
			Try
				...
				prg.Close()
				prg = New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Blocks, False, "Scan step 7 of 7: Processing verified roms", 0, ar_Output.Length, True)
				prg.Start()
				...
				sSQL &= "			AND File IN" & ControlChars.CrLf
		...
		DevExpress.XtraEditors.XtraMessageBox.Show("Import successful!", "Import successful", MessageBoxButtons.OK, MessageBoxIcon.Information)
	...
	Private Function Scan_Mame_Gameinfos(ByRef tran As SQLite.SQLiteTransaction, ByRef dt_Mame_Roms As DS_ML.tbl_Mame_RomsDataTable) As Boolean
		...
		Dim prg As New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Marquee, False, "Scan step 1 of 7: Reading game infos from M.A.M.E.", 0, 100, True)
		prg.Start()

		If Alphaleonis.Win32.Filesystem.Directory.Exists(xml_path) Then
			xml_path &= "\" & "mame_" & DateTime.Now.ToString("yyyyMMdd_HHmmss") & ".xml"
		Else
			DevExpress.XtraEditors.XtraMessageBox.Show("Could not find the temporary directory, please set one up in the Settings section.")
			Return False
		End If

		'Dim prg As New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname, 400, 60, ProgressBarStyle.Marquee, False, "Scan step 1 of 7: Reading game infos from M.A.M.E.", 0, 100, True)
		'prg.Start()
		...
		If Not Alphaleonis.Win32.Filesystem.File.Exists(xml_path) Then
			prg.Close()
			DevExpress.XtraEditors.XtraMessageBox.Show("Could not read " & xml_path & "!")
			Return False
		End If
		...
		prg.Close()
		prg = New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Blocks, False, "Scan step 2 of 7: Processing game infos from M.A.M.E. ({0} / {1})", 0, GameCount, True)
		prg.Start()
		...
		If Alphaleonis.Win32.Filesystem.File.Exists(txb_history_dat.Text) Then
			prg = New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Blocks, False, "Scan step 3 of 7: Reading game infos from history.dat", 0, 100, True)
			prg.Start()
		...
		If Alphaleonis.Win32.Filesystem.File.Exists(txb_mameinfo_dat.Text) Then
			...
			prg = New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Blocks, False, "Scan step 4 of 7: Reading game infos from mameinfo.dat ({0} / {1})", 0, 100, True)
			prg.Start()
		...
		prg = New MKNetDXLib.cls_MKDXBaseform_Progress_Helper(cls_Skins.GetCurrentSkinname(tran), 400, 60, ProgressBarStyle.Blocks, False, "Scan step 5 of 7: Save to internal table ({0} / {1})", 0, dt_Mame_Roms.Rows.Count, True)
		prg.Start()
		...
		'	Me.Cursor = Cursors.Default
		'	If prg IsNot Nothing Then prg.Close()
		'	DevExpress.XtraEditors.XtraMessageBox.Show("Exception: " & ex.Message, "ERROR during scan", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
		'	Return False
		'End Try
		...
		While i < aContent.Length
			...
			If aContent(i).Trim.StartsWith("[") AndAlso Not aContent(i).Trim.ToLower = "[folder_settings]" Then
				...
				If al_RomNames.Count > 0 Then
					...
					End If
				Else
					DevExpress.XtraEditors.XtraMessageBox.Show(sCategory & " does not contain any roms", "Empty ini Folder", MessageBoxButtons.OK, MessageBoxIcon.Information)
				End If
	...
	Private Sub Import_Ini_File_Assignmentbased(ByRef aContent As String(), ByVal iniFile As String)
		...
		For Each catroms As cls_Mame_Category_Roms In al_Mame_Category_Roms
			If catroms.RomNames.Count > 0 Then
				...
				End If
			Else
				DevExpress.XtraEditors.XtraMessageBox.Show(catroms.Category & " does not contain any roms", "Empty ini Folder", MessageBoxButtons.OK, MessageBoxIcon.Information)
			End If
		Next
	End Sub
	...
	Private Sub btn_ini_Folder_Import_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_ini_Folder_Import.Click
		...
		If Alphaleonis.Win32.Filesystem.File.Exists(iniFile) Then
			...
			DevExpress.XtraEditors.XtraMessageBox.Show("The import completed.", "Import completed", MessageBoxButtons.OK, MessageBoxIcon.Information)
